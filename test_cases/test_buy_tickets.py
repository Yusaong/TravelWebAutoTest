...
def test_buy_tickets(get_login_page, get_ticket_page, get_order_page, data):
    with allure.step('初始化驱动器、页面对象'):
        login = get_login_page
        ticket = get_ticket_page
        order = get_order_page
        driver = login.driver
    ...
    with allure.step('进入购票流程'):
        ticket.search_tickets(start_station, arrive_station, time)
        order.order_tickets(name ,id_card, phone, protocol)
    # 如果购票成功，判断购票成功
    if success:
        assert_with_screenshot('true', driver, order.submit_access, None)
    else:
        msg = order.buy_get_error_info()
